...
class Conversation extends Connector
{
    ...
    public static function gets($token, $params = array())
    {
        ...
        return parent::retrieve_list(get_class(), base_url('texts').get_path('texts', 'conversations'), $token, json_encode($params));
    }

    public static function find_id($token, $id)
    {
        return parent::retrieve_once(get_class(), base_url('texts').get_path('texts', 'conversations'), $token, $id, true);
    }
}
